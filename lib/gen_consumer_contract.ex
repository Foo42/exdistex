defmodule Exdistex.GenConsumerContract do
	defmacro __using__(_) do
		quote location: :keep do
			...
			def start_link(context_expression, amqp_channel) do
				GenServer.start_link(__MODULE__, %{context_expression: context_expression, amqp_channel: amqp_channel})
			end

			def init(state = %{amqp_channel: chan, context_expression: context_expression}) do
		        token = "#{:random.uniform}" |> String.replace ".", ""
		        queue_name = "consumer_contract-" <> token
		        ...
		        AMQP.Basic.publish chan, "distex", "event.handler.required", "#{Poison.Encoder.encode(message, [])}"

            #Question should we be doing this here, or should we push out the request from within the GenConsumer?
            #Consumer has additional complexity that it needs to subscribe to the request  channel, and then
            # stop once it has a proposals in from the providers and begin listening on their channels.

		        new_state = state
		        	|> Dict.put(:request_token, token)
